...
(function appendCoindeskData(){
  ...
  $.getJSON( "http://api.coindesk.com/v1/bpi/currentprice/USD.json", function( data ) {
    var currentprice = (Math.round(data.bpi.USD.rate_float*100)/100).toFixed(2),
      updated = new Date(data.time.updatedISO),
      updatedlocale = updated.toLocaleString().replace(/:00$/,'');
    $( ".js-usd-value" ).empty().append( currentprice );
